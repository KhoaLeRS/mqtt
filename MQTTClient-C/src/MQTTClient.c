...
//#include <stdio.h>
//#include <string.h>
#include <printf_master/printf.h>
#include "conf_sys.h"
...
static void NewMessageData(MQTT_MessageData* md, MQTTString* aTopicName, MQTTMessage* aMessage) {
...
void MQTTClientInit(MQTT_Client* c, Network* network, unsigned int command_timeout_ms,
    ...
    if (TimerIsExpired(&c->last_sent) || TimerIsExpired(&c->last_received))
    {
        ...
        } else
        {
            ...
            if (len > 0 && (rc = sendPacket(c, len, &timer)) == MQTT_SUCCESS) // send the ping packet
            	c->ping_outstanding++;//c->ping_outstanding = 1;
            //debugPush(1,xTaskGetTickCount());
        }
    }
...
void MQTTRun(void* parm){
	...
	{
#if defined(NETWORK_TASK)
		//Wait for signal from usart timeout for about 90% of the timeout
		//xSemaphoreTake(c->mutex.rdSem, MQTT_TASK_KEEP_ALIVE/*(c->keepAliveInterval * 9/10) * 1000*/);
		vTaskDelayUntil(&xLastWakeTime, MQTT_TASK_KEEP_ALIVE / portTICK_RATE_MS);

		//wait of there are any activated block (e.g. sub...)
		MutexLock(&c->mutex);
#endif
		TimerCountdownMS(&timer, MQTT_TASK_TIMEOUT / portTICK_RATE_MS); /* Don't wait too long if no traffic is incoming */
		cycle(c, &timer);
#if defined(NETWORK_TASK)
		MutexUnlock(&c->mutex);
#endif
...
int MQTTStartTask(MQTT_Client* client) {
	int ret;
	if (client != 0 && client->thread.task == 0) {
		MutexLock(&client->mutex);
		ret = ThreadStart(&client->thread, &MQTTRun, client);
...
void MQTTStopTask(MQTT_Client* client) {
	if (client != 0) {
		//Must acquire mutex first
		MutexLock(&client->mutex);
...
static int waitfor(MQTT_Client* c, int packet_type, Timer* timer)
{
    int rc = MQTT_FAILURE;
    ...
    do
    {
        ...
        rc = cycle(c, timer);
        xLastWakeTime = xTaskGetTickCount();
        vTaskDelayUntil(&xLastWakeTime, 400 / portTICK_RATE_MS);//Wait between each delay
    }
...
int MQTTConnectWithResults(MQTT_Client* c, MQTTPacket_connectData* options, MQTT_ConnackData* data)
{
    Timer connect_timer;
    int rc = MQTT_FAILURE;
...
{
    MQTT_ConnackData data;
    return MQTTConnectWithResults(c, options, &data);
}


static int SetMessageHandler(MQTT_Client* c, const char* topicFilter, mqtt_messageHandler messageHandler)
{
    int rc = MQTT_FAILURE;
    int i = -1;
...
int MQTTSubscribeWithResults(MQTT_Client* c, const char* topicFilter, enum mqtt_QoS qos,
       mqtt_messageHandler messageHandler, MQTT_SubackData* data)
{
    int rc = MQTT_FAILURE;
    Timer timer;
    int len = 0;
    ...
    if (waitfor(c, SUBACK, &timer) == SUBACK)      // wait for suback
    {
        ...
        if (MQTTDeserialize_suback(&mypacketid, 1, &count, (int*)&data->grantedQoS, c->readbuf, c->readbuf_size) == 1)
        {
            if (data->grantedQoS != 0x80)
                rc = SetMessageHandler(c, topicFilter, messageHandler);
        }
    }
    else
        rc = MQTT_FAILURE;
...
int MQTTSubscribe(MQTT_Client* c, const char* topicFilter, enum mqtt_QoS qos,
       mqtt_messageHandler messageHandler)
{
    MQTT_SubackData data;
...
int MQTTUnsubscribe(MQTT_Client* c, const char* topicFilter)
{
    ...
    if (waitfor(c, UNSUBACK, &timer) == UNSUBACK)
    {
        ...
        if (MQTTDeserialize_unsuback(&mypacketid, c->readbuf, c->readbuf_size) == 1)
        {
            ...
            SetMessageHandler(c, topicFilter, NULL);
        }
    }
    else
        rc = MQTT_FAILURE;
...
int MQTTPing(MQTT_Client* c) {
	...
	if (waitfor(c, PINGRESP, &timer) != PINGRESP) {
		rc = MQTT_FAILURE;
	}
